...
def plot_artemisia_presence_absence():
    """
    Create a Folium map showing Artemisia presence and absence points
    """
    # Read the data
    presence_df = pd.read_csv('outputs/extracted_features/artemisia_presence_features.csv')
    absence_df = pd.read_csv('outputs/extracted_features/artemisia_absence_features.csv')
    ...
    for idx, row in presence_df.iterrows():
        if not pd.isna(row['longitude']) and not pd.isna(row['latitude']):
            ...
            popup_text = f"""
            <b>Macaranga Presence Point</b><br>
    ...
    for idx, row in absence_df.iterrows():
        if not pd.isna(row['longitude']) and not pd.isna(row['latitude']):
            # Create popup with information
            popup_text = f"""
            <b>Macaranga Absence Point</b><br>
            <b>Coordinates:</b> {row['latitude']:.4f}, {row['longitude']:.4f}<br>
            <b>Elevation:</b> {row.get('elevation', 'N/A'):.2f}<br>
            <b>Annual Mean Temperature:</b> {row.get('annual_mean_temperature', 'N/A'):.3f}<br>
            <b>Annual Precipitation:</b> {row.get('annual_precipitation', 'N/A'):.3f}<br>
    ...
    <p><b>Macaranga Distribution</b></p>
    ...
    <p><b>Macaranga Dataset Statistics</b></p>
    ...
    # Save the map
    output_file = 'artemisia_presence_absence_map.html'
    m.save(output_file)
...
if __name__ == "__main__":
    # Create the map
    map_obj = plot_artemisia_presence_absence()
    print("\nMap created successfully! Open 'artemisia_presence_absence_map.html' in your browser to view it.")
