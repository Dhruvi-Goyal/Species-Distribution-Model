...
def dice_grad(y, p, smooth=1):
    # Dice = 1 - (2 * TP) / (|y|+|p|)
    intersection = np.sum(y * p)
    union = np.sum(y) + np.sum(p)
    # d(Dice)/dp = (2*y*union - 2*intersection*1) / union^2
    grad = -(2 * (y * union - intersection)) / (union**2 + smooth)
    return grad

def focal_grad(y, p, alpha=0.25, gamma=2.0, eps=1e-7):
    p = np.clip(p, eps, 1 - eps)
    pt = np.where(y == 1, p, 1 - p)
    af = np.where(y == 1, alpha, 1 - alpha)
    # ∂FL/∂p = -(α * (1-pt)^γ * ( (1-pt)*γ*(-1) * log(pt) + 1/pt ) * sign ) 
    grad = af * (p - y) * ( (1 - pt)**gamma ) * (gamma * np.log(pt) + 1)
    return grad

def tversky_grad(y, p, alpha=0.7, beta=0.3, smooth=1):
    TP = np.sum(y * p)
    FP = np.sum((1-y) * p)
    FN = np.sum(y * (1-p))
    denom = TP + alpha * FP + beta * FN + smooth
    # derivative wrt p:
    grad = -( (y * denom - (TP) * (y + alpha*(1-y))) ) / (denom**2)
    return grad
...
class CustomLossRandomForest:
    ...
    def fit(self, X, y, sample_weights=None, max_iter=100, restarts=10):
        ...
        for _ in range(restarts):
            ...
            try:
                print("enteredd")
                res = differential_evolution(obj, bounds=[(0, 1)] * P.shape[1], maxiter=max_iter, polish=True)
                w = np.abs(res.x)
                w /= w.sum()
                print(w)
...
def all_ecoregion_level_genus_with_precomputed_features(genus, features_csv_path="data/presence_points_with_features.csv", optimize_for='tpr', bias_correction=True):
    """
...
if __name__ == "__main__":
    ...
    # species_list = ["Mangifera indica"]
    # perform_feature_importance_for_all_species(species_list)
    
    genus_list = ["Dalbergia"]
    perform_feature_importance_for_all_genera(genus_list)
